		...
		<tr>
			<td>
				<select name="store[]" required="required" class="store_selecting">
					<?php foreach ($stores as $store): ?>
						<option value="<?=$store['id']?>" <?php if($store['id'] == $item['store_id']) echo 'selected';?>><?=$store['name'].' ('.$store['city'].')'; ?></option>
					<?php endforeach; ?>
				</select>
			</td>
			<td>
			...
			<td>
				<select name="material[]">
					...
					<option value="4" <?if($item['material_id']==4) echo 'selected'?>>Citylight</option>
				</select>
				<?php if($i==$count):?>
...
<?php endforeach;?>
	<table class="hidden">
		<tr id="additional_row">
			...
			<td>
				<input type="number" placeholder="Amount" name="amount[]" class="number">
			</td>
			<td>
				<select name="material[]">
					<option value="3">Blueback</option>
